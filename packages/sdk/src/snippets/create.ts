...
function calculateExpiresAt(expiresIn: ExpirationOption): string | undefined {
  ...
  const durations: Record<Exclude<ExpirationOption, 'never'>, number> = {
  ...
  return new Date(Date.now() + durations[expiresIn as Exclude<ExpirationOption, 'never'>]).toISOString()
}
